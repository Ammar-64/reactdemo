...
import { MoviePoster, CastList, TrailerList, RelatedMoviesList} from '../components';
import { CAST_MAX_NUM, TRAILER_MAX_NUM, RELATED_MOVIES_MAX_NUM } from '../const';
...
class MovieDetail extends Component {
  ...
  render() {
    const {movie, casts, trailers, relatedMovies, isFetcing_movie, isFetcing_casts, isFetcing_trailers, isFetcing_relatedMovies} = this.props;

    if(isFetcing_movie || isFetcing_casts || isFetcing_trailers || isFetcing_relatedMovies) {
      return <p>loading...</p>
    }
    ...
    if(movie.hasOwnProperty('id')) {
      return(
        <Grid fluid={false}>
          ...
          <Row>
            <RelatedMoviesList data={relatedMovies.slice(0,RELATED_MOVIES_MAX_NUM)} />
          </Row>
        </Grid>
      );
    } else
...
function mapStateToProps(state){
  ...
  const {isFetcing_relatedMovies, items: relatedMovies, error_relatedMovies} = relatedMoviesList
  console.log(relatedMoviesList);
  

  return {isFetcing_movie, movie, error_movie, isFetcing_casts, casts, error_casts, isFetcing_trailers, trailers, error_trailers, isFetcing_relatedMovies, relatedMovies, error_relatedMovies}
}
